...
public:
	...
	void print();
	int IndexIs(VertexType* vertex1, VertexType vertex);
...
template <class VertexType>
int GraphType<VertexType>::IndexIs(VertexType* vertex1, VertexType vertex)
{
	int index = 0;

	//while (!(vertex == vertex1[index]))
		index++;
	return index;
}
...
bool GraphType<VertexType>::DepthFirstSearch(VertexType startVertex, VertexType endVertex)
{
	...
	//base case
	if (vertex == endVertex)    //기본케이스 만약 vertex가 endVertex값일 경우 출력 후 true반환
	{
		cout << endVertex;
		return true;
	}
	...
	//general case
	vertexQ.Enqueue(startVertex);         //인접 노드들을 vertexQ에 넣음
	while (!vertexQ.IsEmpty())
	{
		vertexQ.Dequeue(vertex);
		if (vertex != startVertex)
		{
			if (DepthFirstSearch(vertex, endVertex))    //vertex는 시작노드.재귀호출
			{
				cout << "<-" << vertex;
				return true;
			}
...
void GraphType<VertexType>::DeleteEdge(VertexType fromVertex, VertexType toVertex)
{
	...
	edges[row][col] = NULL_EDGE; // 초기값 대입
}
